function [V_h, V_l, V_n,W_1,W_3,W_4]=load_example(respiratory_support, TF,options_tf, max_examples,heart_path,lung_path,cry_path,stmv_path,bubble_path,cpap_path,options_nmf,MAXITER,K)
%% Purpose
...
% Noise Examples 
% cry
%[V_n, V_n_pha] = read_files(cry_path,TF,options_tf,[],[],max_examples);
% stethoscope
%[V_n, V_n_pha] = read_files(stmv_path,TF,options_tf, V_n, V_n_pha,max_examples);
%[V_n, V_n_pha] = read_files(stmv_path,TF,options_tf, [], [],max_examples);
% V_n=[];
% V_n_pha=[]; 
%respiratory support
switch respiratory_support
    case "none"
    case "CPAP"
        %[V_n, ~] = read_files(cpap_path,TF,options_tf, V_n, V_n_pha,max_examples);
        [V_n, ~] = read_files(cpap_path,TF,options_tf, [], [],max_examples);
    case "Bubble"
        %[V_n, ~] = read_files(bubble_path,TF,options_tf, V_n, V_n_pha,max_examples);
        [V_n, ~] = read_files(bubble_path,TF,options_tf, [], [],max_examples);
end
