...
function [modes,its]=ceemdan_improved(x,Nstd,NR,MaxIter,SNRFlag)
x=x(:)';
desvio_x=std(x);
x=x/desvio_x;
...
for i=1:NR %calculates the first mode
    ...
    [temp, ~, it]=emd_alt(xi,'MAXMODES',1,'MAXITERATIONS',MaxIter);
    temp=temp(1,:);
    aux=aux+(xi-temp)/NR;
    iter(i,1)=it;
end
...
k=1;
aux=zeros(size(x));
es_imf = min(size(emd_alt(medias(end,:),'MAXMODES',1,'MAXITERATIONS',MaxIter)));
...
while es_imf>1 %calculates the rest of the modes
    for i=1:NR
        ...
        if tamanio(1)>=k+1
            ...
            noise=Nstd*noise;
            try
                [temp,~,it]=emd_alt(medias(end,:)+std(medias(end,:))*noise,'MAXMODES',1,'MAXITERATIONS',MaxIter);
            catch
                it=0; disp('catch 1 '); disp(num2str(k))
                temp=emd_alt(medias(end,:)+std(medias(end,:))*noise,'MAXMODES',1,'MAXITERATIONS',MaxIter);
            end
            temp=temp(end,:);
        else
            try
                [temp, ~, it]=emd_alt(medias(end,:),'MAXMODES',1,'MAXITERATIONS',MaxIter);
            catch
                temp=emd_alt(medias(end,:),'MAXMODES',1,'MAXITERATIONS',MaxIter);
